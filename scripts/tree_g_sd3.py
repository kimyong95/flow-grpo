"""
This is implementation for D-search
"""
...
def main(_):
    ...
    # 2D list, index by [timestep_id, sample_id]
    all_images = [
        [0 for _ in range(config.sample.total_num_samples)]
        for _ in range(config.sample.num_steps)
    ]
    all_rewards = torch.zeros((config.sample.num_steps, config.sample.total_num_samples), device=accelerator.device, dtype=torch.float32)
    for sample_i in range(config.sample.total_num_samples):
        ...
        def callback_fn(self, index, timestep, kwargs):
            ...
            expansion_size = config.sample.expansion_size
            ...
            # global selection
            next_indices = prev_sample_candidates_rewards.topk(config.sample.batch_size).indices
            ...
            prompt_embeds = einops.repeat(prompt_embeds_base, '(two) ... -> (two b) ...', two=2, b = config.sample.batch_size)
            pooled_prompt_embeds = einops.repeat(pooled_prompt_embeds_base, '(two) ... -> (two b) ...', two=2, b = config.sample.batch_size)
            
            # only log one image among config.sample.batch_size
            one_image = images[0]
            all_images[index][sample_i] = one_image
            all_rewards[index][sample_i] = prev_sample_rewards.mean()
    ...
    for t in range(config.sample.num_steps):
        with tempfile.TemporaryDirectory() as tmpdir:
            for i in range(config.sample.total_num_samples):
                image = all_images[t][i]
                pil = Image.fromarray(
            ...
            wandb_images = [
                ...
                for idx, reward in enumerate(all_rewards[t].cpu().numpy().tolist())
            ]
            wandb.log(
                {
                    f"images": wandb_images,
                    "objective_evaluations": config.sample.total_num_samples * config.sample.expansion_size * config.sample.batch_size * (t+1),
                    f"reward_{config.reward_fn.keys()[0]}": all_rewards[t].mean(),
                    f"reward_avg": all_rewards[t].mean(),
                    **({"final_images": wandb_images} if (t+1) == config.sample.num_steps else {}),
                },
                step=t,
            )
